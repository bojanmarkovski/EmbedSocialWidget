var dataJSON, i, output, outputModal = "";
dataJSON = [
  {
...
function loadMore() {
  ...
  currentIndex += 2;
  maxResult = currentIndex * 2;
  output = "";
  for (let i = 0; i < maxResult; i++) {
    output += `
            <div id="widget-box" class="widget-box popup-trigger" data-popup-trigger="${i}">
              <div class="info">
                <img class="profile-img" src="${dataJSON[i].profile_image}" alt="">
                <div>
                    <p class="name">${dataJSON[i].name}</p>
                    <small class="date">${dataJSON[i].date}</small>
                </div>
                <a href="${dataJSON[i].source_link}" target=_blank>
                  <img class="social-icon" src="assets/icons/${dataJSON[i].source_type}.svg" alt="facebook-icon">
                </a>
              </div>
              <div class="box-img">
                <img src="${dataJSON[i].image}" alt="">
              </div>
  ...
  document.getElementById("container-widgets").innerHTML = output;
  if (dataJSON.length <= maxResult) {
...
for (i = 0; i < dataJSON.length; i++) {
  outputModal += 
        `
          <div class="modal-widget-container popup-modal shadow" data-popup-modal="${i}">
            <div class="modal-img">
                <img src="${dataJSON[i].image}" alt="">
            </div>
            <div class="modal-info">
                <div>
                    <img class="social-icon" src="assets/icons/facebook.svg" alt="facebook-icon">
                    <p class="name">${dataJSON[i].name}</p>
                    <small class="date">${dataJSON[i].date}</small>
                </div>
                <hr>
...
document.getElementById("container-modal").innerHTML = outputModal;